 ...
 */
router.get("/restaurants/:restaurantId/addons", (0, featureFlags_1.requireFeature)("addons"), async (req, res) => {
 ...
 */
router.get("/restaurants/:restaurantId/menu-items/:menuItemId/addons", async (req, res) => {
 ...
 * }
 */
router.post("/restaurants/:restaurantId/addons", (0, featureFlags_1.requireFeature)("addons"), async (req, res) => {
    const { restaurantId } = req.params;
    const { menu_item_id, addon_item_id, addon_name, addon_description, regular_price_cents, addon_discount_price_cents, } = req.body;
    ...
    try {
        ...
        const menuItemRes = await db_1.default.query(`
      ...
      `, [
        ...
        if (result.rowCount === 0) {
            ...
            const existing = await db_1.default.query(`SELECT * FROM addons WHERE restaurant_id = $1 AND menu_item_id = $2 AND addon_item_id = $3`, [restaurantId, menu_item_id, addon_item_id]);
 ...
 */
router.patch("/restaurants/:restaurantId/addons/:addonId", (0, featureFlags_1.requireFeature)("addons"), async (req, res) => {
    const { restaurantId, addonId } = req.params;
    const { addon_name, addon_description, regular_price_cents, addon_discount_price_cents, is_available, } = req.body;
    try {
        // Verify addon belongs to restaurant
        const addonRes = await db_1.default.query(`SELECT * FROM addons WHERE id = $1 AND restaurant_id = $2`, [addonId, restaurantId]);
 ...
 */
router.delete("/restaurants/:restaurantId/addons/:addonId", (0, featureFlags_1.requireFeature)("addons"), async (req, res) => {
    const { restaurantId, addonId } = req.params;
    try {
        const result = await db_1.default.query(`
      DELETE FROM addons
