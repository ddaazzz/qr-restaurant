...
const restaurantDefaults_1 = require("../config/restaurantDefaults");
...
router.get("/restaurants/:restaurantId/config", async (req, res) => {
    try {
        ...
        let result;
        try {
            result = await db_1.default.query(`SELECT
        ...
        const featureFlags = (0, restaurantDefaults_1.mergeFeatureFlags)(r.feature_flags, r);
        const uiConfig = (0, restaurantDefaults_1.mergeUiConfig)(r.ui_config);
        res.json({
            id: r.id,
            name: r.name,
            logo_url: r.logo_url,
...
router.patch("/restaurants/:restaurantId/config", async (req, res) => {
    try {
        ...
        try {
            ...
            const userResult = await db_1.default.query("SELECT id, role, restaurant_id FROM users WHERE id = $1", [decoded.id]);
        ...
        const result = await db_1.default.query(`UPDATE restaurants SET ${updates.join(", ")} WHERE id = $${paramCount}
       RETURNING id, feature_flags, ui_config, ui_mode, custom_frontend_url, custom_domain, is_customized`, values);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Restaurant not found" });
        }
