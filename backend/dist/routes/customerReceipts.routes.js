 ...
 */
router.post("/sessions/:sessionId/receipts/send-email", async (req, res) => {
    ...
    try {
        // Get session info
        const sessionRes = await db_1.default.query(`SELECT ts.id, ts.table_name, ts.restaurant_id, r.service_charge_percent
       FROM table_sessions ts
       JOIN restaurants r ON r.id = ts.restaurant_id
       WHERE ts.id = $1 AND ts.restaurant_id = $2`, [sessionId, restaurantId]);
        if ((sessionRes.rowCount ?? 0) === 0) {
            return res.status(404).json({ error: "Session not found" });
        }
        const session = sessionRes.rows[0];
        // Get all non-addon order items for the session
        const itemsRes = await db_1.default.query(`SELECT oi.quantity, oi.price_cents,
