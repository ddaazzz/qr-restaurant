 ...
 */
router.get("/restaurants/:restaurantId/menu_categories", async (req, res) => {
    try {
...
router.patch("/menu_categories/:id", async (req, res) => {
    try {
       ...
       RETURNING id, name, name_zh, sort_order,
                 ...
                 TO_CHAR(available_from, 'HH24:MI') AS available_from,
                 TO_CHAR(available_to,   'HH24:MI') AS available_to,
                 COALESCE(days_of_week, '{}') AS days_of_week`, values);
...
router.delete("/menu_categories/:id", async (req, res) => {
    try {
        ...
        const categoryCheck = await db_1.default.query(`SELECT id FROM menu_categories WHERE id = $1 AND restaurant_id = $2`, [id, restaurantId]);
        if (categoryCheck.rowCount === 0) {
            return res.status(404).json({ error: "Category not found or doesn't belong to this restaurant" });
        }
        const used = await db_1.default.query(`
      SELECT 1
      FROM menu_items
      WHERE category_id = $1
      LIMIT 1
      `, [id]);
        ...
        const result = await db_1.default.query("DELETE FROM menu_categories WHERE id = $1 AND restaurant_id = $2", [id, restaurantId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Category not found" });
        }
...
router.put("/restaurants/:restaurantId/menu-categories/reorder", async (req, res) => {
    try {
        ...
        await Promise.all(categories.map(({ id, sort_order }) => db_1.default.query("UPDATE menu_categories SET sort_order = $1 WHERE id = $2 AND restaurant_id = $3", [sort_order, id, restaurantId])));
        res.json({ ok: true });
    }
    catch (err) {
...
router.put("/restaurants/:restaurantId/menu-items/reorder", async (req, res) => {
    try {
        ...
        await Promise.all(items.map(({ id, sort_order }) => db_1.default.query("UPDATE menu_items SET sort_order = $1 WHERE id = $2", [sort_order, id])));
        res.json({ ok: true });
    }
    catch (err) {
...
router.get("/restaurants/:restaurantId/menu", async (req, res) => {
    ...
    try {
        ...
        let variants = [];
        if (itemIds.length > 0) {
            const variantsResult = await db_1.default.query(`SELECT
                  v.*,
                  o.id AS option_id,
                ...
                WHERE v.menu_item_id = ANY($1::int[])`, [itemIds]);
...
router.get("/restaurants/:restaurantId/menu/staff", async (req, res) => {
    try {
        const { restaurantId } = req.params;
        //Load Menu Items
        const itemsResult = await db_1.default.query(`
      SELECT
      mi.id,
      mi.name,
      mi.name_zh,
      ...
      `, [restaurantId]);
        ...
        const variantsResult = await db_1.default.query(`
  SELECT
    v.id              AS variant_id,
    v.menu_item_id,
  ...
  `, [itemsResult.rows.map(i => i.id)]);
        ...
        for (const row of variantsResult.rows) {
            ...
            let variant = variantsByItem[row.menu_item_id].find(v => v.id === row.variant_id);
            if (!variant) {
                variant = {
                    id: row.variant_id,
...
router.patch("/menu-items/:id/availability", async (req, res) => {
    try {
        ...
        const itemCheck = await db_1.default.query(`SELECT mi.id FROM menu_items mi
       JOIN menu_categories mc ON mi.category_id = mc.id
       WHERE mi.id = $1 AND mc.restaurant_id = $2`, [id, restaurantId]);
...
router.patch("/menu-item-variant-options/:id/availability", async (req, res) => {
    const { id } = req.params;
...
router.post("/restaurants/:restaurantId/menu-items", async (req, res) => {
    try {
        ...
        // Verify category belongs to restaurant
        const categoryCheck = await db_1.default.query(`SELECT id FROM menu_categories WHERE id = $1 AND restaurant_id = $2`, [category_id, restaurantId]);
        if (categoryCheck.rowCount === 0) {
...
router.patch("/menu-items/:itemId", async (req, res) => {
    try {
        ...
        const itemCheck = await db_1.default.query(`SELECT mi.id FROM menu_items mi
       JOIN menu_categories mc ON mi.category_id = mc.id
       WHERE mi.id = $1 AND mc.restaurant_id = $2`, [itemId, restaurantId]);
        if (itemCheck.rowCount === 0) {
            return res.status(404).json({ error: "Menu item not found or doesn't belong to this restaurant" });
        }
        // If updating category, verify new category belongs to same restaurant
        if (category_id) {
            const catCheck = await db_1.default.query(`SELECT id FROM menu_categories WHERE id = $1 AND restaurant_id = $2`, [category_id, restaurantId]);
...
router.delete("/menu-items/:itemId", async (req, res) => {
    try {
       ...
       WHERE mi.id = $1 AND mc.restaurant_id = $2`, [itemId, restaurantId]);
        ...
        await db_1.default.query(`UPDATE order_items SET menu_item_id = NULL WHERE menu_item_id = $1`, [itemId]);
        const result = await db_1.default.query("DELETE FROM menu_items WHERE id = $1", [itemId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Menu item not found" });
        }
        res.json({ success: true });
...
router.get("/menu-items/:menuItemId/variants", async (req, res) => {
    try {
      ...
      `, [menuItemId]);
        ...
        const variants = result.rows.reduce((acc, row) => {
...
router.delete("/variants/:variantId", async (req, res) => {
    try {
        const { variantId } = req.params;
        await db_1.default.query("DELETE FROM menu_item_variant_options WHERE variant_id = $1", [variantId]);
        const result = await db_1.default.query("DELETE FROM menu_item_variants WHERE id = $1", [variantId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Variant not found" });
        }
        res.json({ success: true });
...
router.delete("/variant-options/:id", async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.default.query(`DELETE FROM menu_item_variant_options WHERE id = $1`, [id]);
        res.status(204).end();
    }
    catch (err) {
 ...
 * (Admin)
 */
router.post("/menu-items/:menuItemId/image", upload_1.upload.single("image"), async (req, res) => {
    try {
        const { menuItemId } = req.params;
        ...
        if ((0, storage_1.isR2Configured)() && req.file.buffer) {
            imageUrl = await (0, storage_1.uploadToR2)(req.file.buffer, req.file.originalname, (0, storage_1.getR2Folder)("menu", restaurantId), req.file.mimetype);
        }
        else {
...
router.post("/restaurants/:restaurantId/menu-items/:menuItemId/image", upload_1.upload.single("image"), async (req, res) => {
    try {
        ...
        const itemCheck = await db_1.default.query(`SELECT mi.id FROM menu_items mi
         JOIN menu_categories mc ON mi.category_id = mc.id
         WHERE mi.id = $1 AND mc.restaurant_id = $2`, [menuItemId, restaurantId]);
        if (itemCheck.rowCount === 0) {
            return res.status(404).json({ error: "Menu item not found or doesn't belong to this restaurant" });
        }
        let imageUrl;
...
router.get("/restaurants/:restaurantId/menu/items", async (req, res) => {
    ...
    try {
        const itemsResult = await db_1.default.query(`SELECT mi.*
       FROM menu_items mi
       JOIN menu_categories mc ON mi.category_id = mc.id
       WHERE mc.restaurant_id = $1`, [restaurantId]);
