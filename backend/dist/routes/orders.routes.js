 ...
 */
function getPaymentMethodLabel(method, paid) {
    if (!paid)
        return 'Unpaid';
    if (method === 'kpay')
        return 'Paid (KPay)';
    if (method === 'payment-asia')
        return 'Paid (Payment Asia)';
    return 'Paid';
}
// Place an order
...
router.post("/sessions/:sessionId/orders", async (req, res) => {
    ...
    try {
      ...
      `, [sessionId]);
        ...
        let orderId;
        const existingOrderRes = await db_1.default.query(`
      ...
      `, [sessionId]);
        if ((existingOrderRes.rowCount ?? 0) > 0) {
            ...
            const orderRes = await db_1.default.query(`
      ...
      ------------------------------ */
            ...
            const variantsRes = await db_1.default.query(`
      ...
      `, [optionIds])
            ...
            for (const v of variants) {
                ...
                if (v.min_select !== null && count < v.min_select) {
                    throw new Error(`Must select at least ${v.min_select} option(s)`);
                }
                // MAX
                if (v.max_select !== null && count > v.max_select) {
                    throw new Error(`You can select at most ${v.max_select} option(s)`);
                }
            }
            /* ------------------------------
               Calculate price
        ...
        `, [
          ...
          `, [orderItemId, optionId]);
            ...
            for (const addon of addons) {
                const addonRes = await db_1.default.query(`
          ...
          `, [addon.addon_id, restaurantId]);
                ...
                const addonItemRes = await db_1.default.query(`
          INSERT INTO order_items
          ...
          `, [
                ...
                for (const optionId of addonOptionIds) {
                    await db_1.default.query(`INSERT INTO order_item_variants (order_item_id, variant_option_id) VALUES ($1, $2)`, [addonOrderItemId, optionId]);
                }
            }
        }
        // ✅ AUTO-PRINT: Check if kitchen auto-print is enabled
        try {
            const restaurantConfigRes = await db_1.default.query(`SELECT kitchen_auto_print, printer_type FROM restaurant_printer_settings WHERE restaurant_id = $1`, [restaurantId]);
            if ((restaurantConfigRes.rowCount ?? 0) > 0) {
                ...
                if (config.kitchen_auto_print && config.printer_type && config.printer_type !== 'none') {
                    ...
                    const orderDetailsRes = await db_1.default.query(`
            SELECT 
              oi.id,
            ...
            `, [orderId, restaurantId]);
                    if ((orderDetailsRes.rowCount ?? 0) > 0) {
                        ...
                        const zonesService = (0, printerZones_1.getPrinterZonesService)(db_1.default);
                        const queue = (0, printer_routes_1.getPrinterQueueInstance)();
        ...
        res.status(err.message?.includes("select") ? 400 : 500).json({ error: err.message });
    }
});
      ...
      `, [sessionId]);
        ...
        for (const row of result.rows) {
            ...
            if (!row.order_item_id)
                continue;
            const itemTotal = Number(row.unit_price_cents) * Number(row.quantity);
            const itemObj = {
...
router.patch("/order-items/:orderItemId/status", async (req, res) => {
    ...
    try {
        ...
        const orderCheck = await db_1.default.query(`SELECT oi.id FROM order_items oi
       JOIN orders o ON oi.order_id = o.id
       JOIN table_sessions ts ON o.session_id = ts.id
       WHERE oi.id = $1 AND ts.restaurant_id = $2`, [orderItemId, restaurantId]);
...
router.patch("/order-items/:id", async (req, res) => {
    ...
    try {
        // Verify order item belongs to restaurant
        const checkRes = await db_1.default.query(`SELECT oi.id FROM order_items oi
       JOIN orders o ON o.id = oi.order_id
       JOIN table_sessions ts ON ts.id = o.session_id
       WHERE oi.id = $1 AND ts.restaurant_id = $2`, [id, restaurantId]);
        ...
        const orderIdRes = await db_1.default.query(`SELECT order_id FROM order_items WHERE id = $1`, [id]);
        const orderId = orderIdRes.rows[0]?.order_id;
        if (quantity <= 0) {
            await db_1.default.query(`DELETE FROM order_items WHERE id = $1`, [id]);
        ...
        `, [orderId]);
            ...
            await db_1.default.query(`UPDATE order_items
         SET quantity = $1
...
router.post("/sessions/:sessionId/close", async (req, res) => {
    try {
        ...
        const sessionResult = await db_1.default.query(`SELECT ts.id, t.restaurant_id FROM table_sessions ts
     JOIN tables t ON t.id = ts.table_id
     WHERE ts.id = $1 AND ts.ended_at IS NULL AND t.restaurant_id = $2`, [sessionId, restaurantId]);
        if (sessionResult.rowCount === 0) {
            return res.status(404).json({ error: "Session not found, already closed, or doesn't belong to this restaurant" });
        }
        // Close session
        await db_1.default.query("UPDATE table_sessions SET ended_at = NOW() WHERE id = $1", [sessionId]);
...
router.get("/restaurants/:restaurantId/orders/:orderId", async (req, res) => {
    try {
      ...
      `, [orderId, restaurantId]);
        ...
        const itemsRes = await db_1.default.query(`
      SELECT
        oi.id,
        oi.order_id,
        oi.menu_item_id,
      ...
      `, [orderId]);
        ...
        const paymentsLedgerRes = await db_1.default.query(`SELECT
        id,
        payment_vendor,
        payment_method,
        payment_gateway_env,
...
router.post("/restaurants/:restaurantId/orders/:orderId/void", async (req, res) => {
    try {
        const { restaurantId, orderId } = req.params;
        const orderRes = await db_1.default.query(`SELECT o.id, o.payment_method, o.payment_status
       FROM orders o
       WHERE o.id = $1 AND o.restaurant_id = $2`, [orderId, restaurantId]);
        if (orderRes.rowCount === 0)
            return res.status(404).json({ error: "Order not found" });
        const order = orderRes.rows[0];
        ...
        await db_1.default.query(`UPDATE orders SET payment_status = 'voided' WHERE id = $1`, [orderId]);
...
router.post("/restaurants/:restaurantId/orders/:orderId/refund", async (req, res) => {
    try {
       ...
       WHERE o.id = $1 AND o.restaurant_id = $2`, [orderId, restaurantId]);
        ...
        await db_1.default.query(`UPDATE orders SET payment_status = $1 WHERE id = $2`, [newStatus, orderId]);
...
router.post("/restaurants/:restaurantId/orders/:orderId/send-receipt", async (req, res) => {
    ...
    try {
        ...
        const orderRes = await db_1.default.query(`SELECT id, restaurant_order_number FROM orders WHERE id = $1 AND restaurant_id = $2`, [orderId, restaurantId]);
        if (orderRes.rowCount === 0) {
            return res.status(404).json({ error: "Order not found" });
        }
        const order = orderRes.rows[0];
        // Get restaurant name for email
        const restaurantRes = await db_1.default.query(`SELECT name FROM restaurants WHERE id = $1`, [restaurantId]);
        const restaurantName = restaurantRes.rows[0]?.name || "Restaurant";
        // Send receipt email
        const result = await (0, emailService_1.sendReceipt)({
            toEmail: email,
...
router.get("/restaurants/:restaurantId/reports/export", async (req, res) => {
    try {
        const { restaurantId } = req.params;
        const { date_from, date_to, period, period_from, period_to, order_type, pax_min, pax_max } = req.query;
        // Period presets
        const PERIODS = {
        ...
        if (timeFrom && timeTo) {
            ...
            conditions.push(`o.created_at::time >= $${params.length}::time`);
            params.push(timeTo);
            conditions.push(`o.created_at::time <= $${params.length}::time`);
        }
        // Dining type
        if (order_type) {
            const types = order_type.split(',').map((t) => t.trim()).filter(Boolean);
            if (types.length > 0) {
                params.push(types);
        ...
        // Pax
        if (pax_min) {
            params.push(parseInt(pax_min));
            conditions.push(`ts.pax >= $${params.length}`);
        }
        if (pax_max) {
            params.push(parseInt(pax_max));
            conditions.push(`ts.pax <= $${params.length}`);
        }
