...
router.get('/restaurants/:restaurantId/orders/:orderId/payment-status', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query(`SELECT id, payment_status, transaction_id, error_message, completed_at, created_at
       FROM order_payments
       ...
       LIMIT 1`, [orderId, restaurantId]);
        ...
        if (payment.payment_status === 'pending') {
           ...
           WHERE id = $1 AND restaurant_id = $2 AND status != 'completed'`, [orderId, restaurantId]);
                await db_1.default.query(`UPDATE order_payments SET payment_status = 'failed', error_message = 'Payment timed out — no response from gateway',
           failed_at = NOW(), updated_at = NOW() WHERE id = $1`, [payment.id]);
