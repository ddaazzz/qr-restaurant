...
let printerQueue = null;
/**
 * Initialize printer queue service (call this in app startup)
 */
function initializePrinterQueue(config) {
    if (!printerQueue) {
        printerQueue = new printerQueue_1.PrinterQueueService(db_1.default, config);
 ...
 */
function getPrinterQueueInstance() {
    if (!printerQueue) {
...
router.get("/restaurants/:restaurantId/printer-settings", async (req, res) => {
    ...
    try {
        ...
        created_at,
        updated_at
       FROM printers WHERE restaurant_id = $1 ORDER BY type`, [restaurantId]);
    ...
    try {
        ...
        if (type) {
         ...
         RETURNING *`, [restaurantId, type, printer_type, printer_host, printer_port || 9100, bluetooth_device_id, bluetooth_device_name, menu_category_id, JSON.stringify(settingsWithEnabled)]);
            ...
            const result = await db_1.default.query(`SELECT * FROM printers WHERE restaurant_id = $1 ORDER BY type`, [restaurantId]);
...
router.post("/restaurants/:restaurantId/printer-test", async (req, res) => {
    ...
    try {
        ...
        bluetooth_device_id,
        bluetooth_device_name
       FROM restaurant_printer_settings WHERE restaurant_id = $1`, [restaurantId]);
        ...
        const testResult = await (0, printerService_1.testPrinterConnection)({
...
router.post("/restaurants/:restaurantId/print-order", async (req, res) => {
    ...
    try {
        ...
        if (printerConfig.printer_type === "bluetooth") {
            ...
            const escposArray = (0, thermalPrinterService_1.generateKitchenOrderESCPOS)(kitchenOrderData);
            return res.json({
                success: true,
                bluetoothPayload: {
                    printerConfig: {
 ...
 */
router.post("/restaurants/:restaurantId/preview-qr", async (req, res) => {
    const { restaurantId } = req.params;
    const { tableName = 'T02', pax = 4, qrTextAbove, qrTextBelow } = req.body;
    try {
        // Get restaurant name
        const restaurantResult = await db_1.default.query(`SELECT name FROM restaurants WHERE id = $1`, [restaurantId]);
        ...
        const settingsResult = await db_1.default.query(`SELECT 
        COALESCE((settings->>'qr_text_above'), '') as text_above,
        COALESCE((settings->>'qr_text_below'), '') as text_below
       FROM printers WHERE restaurant_id = $1 AND type = $2`, [restaurantId, 'QR']);
        if (((settingsResult.rowCount ?? 0) > 0)) {
            const settings = settingsResult.rows[0];
            if (settings.text_above)
                textAboveQR = settings.text_above;
            if (settings.text_below)
                textBelowQR = settings.text_below;
        }
        ...
        const escposArray = (0, thermalPrinterService_1.generateESCPOS)(receiptData);
 ...
 */
router.post("/restaurants/:restaurantId/test-print-qr", async (req, res) => {
    const { restaurantId } = req.params;
 ...
 */
router.post("/restaurants/:restaurantId/preview-bill", async (req, res) => {
    const { restaurantId } = req.params;
    const { headerText = 'Thank You', footerText = 'Follow us on social media' } = req.body;
    try {
        // Get restaurant info
        const restaurantResult = await db_1.default.query(`SELECT name, address, phone FROM restaurants WHERE id = $1`, [restaurantId]);
        const restaurantName = ((restaurantResult.rowCount ?? 0) > 0) ? restaurantResult.rows[0].name : 'La Cave Restaurant';
       ...
       FROM printers WHERE restaurant_id = $1 AND type = $2`, [restaurantId, 'BILL']);
        if (((settingsResult.rowCount ?? 0) > 0)) {
            const settings = settingsResult.rows[0];
            if (settings.header_text)
                billHeaderText = settings.header_text;
            if (settings.footer_text)
                billFooterText = settings.footer_text;
        }
 ...
 * Useful for testing the thermalPrinterService output directly
 */
router.post("/restaurants/:restaurantId/test-print-bill", async (req, res) => {
    const { restaurantId } = req.params;
    try {
        // Get restaurant info
        const restaurantResult = await db_1.default.query(`SELECT name, address, phone FROM restaurants WHERE id = $1`, [restaurantId]);
 ...
 */
router.post("/restaurants/:restaurantId/print-qr", async (req, res) => {
    const restaurantId = req.params.restaurantId;
    ...
    try {
        ...
        let printerResult = await db_1.default.query(`SELECT printer_type, printer_host, printer_port,
       ...
       FROM printers WHERE restaurant_id = $1 AND type = $2`, [restaurantId, 'QR']);
        ...
        if (!printerConfig) {
            ...
            const oldSchemaResult = await db_1.default.query(`SELECT rps.qr_printer_type as printer_type, 
         ...
         WHERE rps.restaurant_id = $1`, [restaurantId]);
      ...
      `;
        ...
        if (printerConfig.printer_type === 'bluetooth') {
            ...
            let pax = undefined;
            if (sessionId) {
                const sessionResult = await db_1.default.query(`SELECT ts.started_at, ts.pax, t.name as table_name 
 ...
 */
router.post("/restaurants/:restaurantId/print-bill", async (req, res) => {
    const restaurantId = req.params.restaurantId;
    ...
    try {
       ...
       FROM printers WHERE restaurant_id = $1 AND type = $2`, [restaurantId, 'Bill']);
        ...
        const restaurantMetaResult = await db_1.default.query(`SELECT name, address, phone FROM restaurants WHERE id = $1`, [restaurantId]);
        ...
        if (!printerConfig) {
            ...
            const oldSchemaResult = await db_1.default.query(`SELECT rps.printer_type, rps.printer_host, rps.printer_port,
 ...
 */
router.post("/restaurants/:restaurantId/register-bluetooth-device", async (req, res) => {
    const { restaurantId } = req.params;
    ...
    try {
        ...
        const result = await db_1.default.query(`UPDATE printers 
       SET bluetooth_device_id = $1,
           bluetooth_device_name = $2,
           service_uuid = $3,
           updated_at = now()
       WHERE restaurant_id = $4 AND type = $5
       RETURNING *`, [deviceId, deviceName, serviceUuid, restaurantId, normalizedType]);
        if (result.rowCount === 0) {
            // No printer config exists for this type, create one
            try {
                const createResult = await db_1.default.query(`INSERT INTO printers (restaurant_id, type, printer_type, bluetooth_device_id, bluetooth_device_name, service_uuid, settings)
           VALUES ($1, $2, 'bluetooth', $3, $4, $5, '{}')
           RETURNING *`, [restaurantId, normalizedType, deviceId, deviceName, serviceUuid]);
 ...
 */
router.get("/restaurants/:restaurantId/bluetooth-devices", async (req, res) => {
    const { restaurantId } = req.params;
    try {
        const result = await db_1.default.query(`SELECT id, type, bluetooth_device_id as device_id, bluetooth_device_name as device_name, service_uuid
       FROM printers
       WHERE restaurant_id = $1 AND printer_type = 'bluetooth'
       ORDER BY updated_at DESC`, [restaurantId]);
        res.json(result.rows);
    }
    catch (err) {
 ...
 */
router.get("/restaurants/:restaurantId/bluetooth-device-uuid/:printerType", async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT bluetooth_device_id as device_id, 
              bluetooth_device_name as device_name,
              service_uuid
       FROM printers
       WHERE restaurant_id = $1 AND type = $2`, [restaurantId, normalizedType]);
 ...
 */
router.post("/restaurants/:restaurantId/preview-kpay-receipt", async (req, res) => {
    const { restaurantId } = req.params;
    try {
        const restaurantResult = await db_1.default.query(`SELECT name FROM restaurants WHERE id = $1`, [restaurantId]);
        ...
        const escposArray = (0, thermalPrinterService_1.generateKPayReceiptESCPOS)(sampleData);
        return res.json({
            success: true,
            escposArray: Array.from(escposArray),
            escposBase64: Buffer.from(escposArray).toString('base64'),
 ...
 */
router.post("/restaurants/:restaurantId/test-print-kpay-receipt", async (req, res) => {
    const { restaurantId } = req.params;
    try {
        const restaurantResult = await db_1.default.query(`SELECT name FROM restaurants WHERE id = $1`, [restaurantId]);
        const restaurantName = ((restaurantResult.rowCount ?? 0) > 0) ? restaurantResult.rows[0].name : 'Restaurant';
        ...
        const escposArray = (0, thermalPrinterService_1.generateKPayReceiptESCPOS)(testData);
        return res.json({
            success: true,
            type: 'test',
            escposArray: Array.from(escposArray),
 ...
 */
router.post("/restaurants/:restaurantId/print-kpay-receipt", async (req, res) => {
    const { restaurantId } = req.params;
    ...
    try {
        ...
        const printerResult = await db_1.default.query(`SELECT printer_type, printer_host, printer_port, bluetooth_device_id, bluetooth_device_name
       FROM printers WHERE restaurant_id = $1 AND type = $2`, [restaurantId, 'KPAY']);
        ...
        const escposArray = (0, thermalPrinterService_1.generateKPayReceiptESCPOS)(receiptData);
        if (printerConfig.printer_type === 'bluetooth') {
            return res.json({
                success: true,
                bluetoothPayload: {
