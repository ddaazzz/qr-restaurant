...
router.get("/:restaurantId/subscription", async (req, res) => {
    try {
        const { restaurantId } = req.params;
        const result = await db_1.default.query("SELECT subscription_tier, subscription_trial_end FROM restaurants WHERE id = $1", [restaurantId]);
...
router.post("/:restaurantId/subscription", async (req, res) => {
    try {
        ...
        try {
            ...
            const userResult = await db_1.default.query("SELECT id, role FROM users WHERE id = $1", [decoded.id]);
        ...
        const { restaurantId } = req.params;
        const { tier, trial_end_date } = req.body;
        if (!tier || !['free', 'premium'].includes(tier)) {
            return res.status(400).json({ error: "tier must be 'free' or 'premium'" });
        }
        await db_1.default.query("UPDATE restaurants SET subscription_tier = $1, subscription_trial_end = $2 WHERE id = $3", [tier, trial_end_date ?? null, restaurantId]);
