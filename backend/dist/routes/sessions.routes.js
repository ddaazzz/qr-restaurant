...
router.patch("/table-sessions/:sessionId/move-table", async (req, res) => {
    ...
    try {
        ...
        const sessionRes = await db_1.default.query(`SELECT ts.id, ts.ended_at, t.restaurant_id
       FROM table_sessions ts
       JOIN tables t ON t.id = ts.table_id
       WHERE ts.id = $1`, [sessionId]);
        if (sessionRes.rowCount === 0)
        ...
        const targetRes = await db_1.default.query(`SELECT id FROM tables WHERE id = $1 AND restaurant_id = $2`, [new_table_id, restaurantId]);
        if (targetRes.rowCount === 0) {
            return res.status(404).json({ error: "Target table not found" });
        }
        // Verify target table has no active sessions
        const occupiedRes = await db_1.default.query(`SELECT id FROM table_sessions WHERE table_id = $1 AND ended_at IS NULL`, [new_table_id]);
        ...
        const result = await db_1.default.query(`UPDATE table_sessions SET table_id = $1 WHERE id = $2 RETURNING id, table_id`, [new_table_id, sessionId]);
    ...
    try {
       ...
       WHERE session_id = $2`, [payment_method, sessionId]);
        ...
        if (payment_method === 'payment-asia-offline' && kpay_reference_id) {
            await client.query(`UPDATE pa_offline_transactions
         SET status = 'completed', completed_at = NOW() AT TIME ZONE 'UTC',
             chuio_order_reference = $1
         WHERE pa_order_id = $2 AND restaurant_id = $3`, [kpay_reference_id, kpay_reference_id, restaurantId]);
            await client.query(`UPDATE orders SET chuio_order_reference = $1 WHERE session_id = $2`, [kpay_reference_id, sessionId]);
        }
        // Free up table
        if (session.table_id) {
...
router.post("/sessions/:sessionId/split-bill/init", async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        const sessionRes = await client.query(`SELECT ts.id, ts.restaurant_id FROM table_sessions ts WHERE ts.id = $1 AND ts.ended_at IS NULL`, [sessionId]);
        if (sessionRes.rowCount === 0) {
            await client.query("ROLLBACK");
        ...
        const billRes = await client.query(`SELECT
       ...
       WHERE o.session_id = $1 AND o.status <> 'cancelled'
       GROUP BY r.service_charge_percent`, [sessionId, restaurant_id]);
        ...
        for (let i = 1; i <= split_count; i++) {
            ...
            const r = await client.query(`INSERT INTO split_bill_payments
           (session_id, restaurant_id, split_index, split_count, amount_cents, service_charge)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [sessionId, restaurant_id, i, split_count, amount, service_charge_cents]);
            splits.push(r.rows[0]);
        }
        // Record split metadata on the session
        await client.query(`UPDATE table_sessions SET split_count = $1, split_bills_paid = 0 WHERE id = $2`, [split_count, sessionId]);
        await client.query("COMMIT");
...
router.get("/sessions/:sessionId/split-bill", async (req, res) => {
    ...
    if (!sessionId)
        return res.status(400).json({ error: "Invalid session id" });
    try {
        const result = await db_1.default.query(`SELECT * FROM split_bill_payments WHERE session_id = $1 ORDER BY split_index ASC`, [sessionId]);
        res.json(result.rows);
    }
    catch (err) {
...
router.post("/sessions/:sessionId/split-bill/:splitIndex/pay", async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        const splitRes = await client.query(`SELECT * FROM split_bill_payments WHERE session_id = $1 AND split_index = $2`, [sessionId, splitIndex]);
        ...
        // Mark portion paid
        await client.query(`UPDATE split_bill_payments
       SET payment_method = $1, notes = $2, closed_at = NOW() AT TIME ZONE 'UTC', closed_by_staff_id = $3
       WHERE id = $4`, [payment_method, notes || null, closed_by_staff_id || null, split.id]);
        // Increment paid count on session
        await client.query(`UPDATE table_sessions SET split_bills_paid = COALESCE(split_bills_paid, 0) + 1 WHERE id = $1`, [sessionId]);
        // Check if all portions are now paid
        const countRes = await client.query(`SELECT split_count, split_bills_paid FROM table_sessions WHERE id = $1`, [sessionId]);
        ...
        if (nowPaid >= Number(split_count)) {
            ...
            const totalRes = await client.query(`SELECT COALESCE(SUM(amount_cents), 0) AS total FROM split_bill_payments WHERE session_id = $1`, [sessionId]);
            const amountPaid = Number(totalRes.rows[0].total);
            await client.query(`UPDATE table_sessions
         SET ended_at = NOW() AT TIME ZONE 'UTC', payment_method = $1, amount_paid = $2
         WHERE id = $3`, [payment_method, amountPaid, sessionId]);
            await client.query(`UPDATE orders
         SET status = 'completed', payment_method = $1, payment_status = 'paid'
         WHERE session_id = $2 AND status <> 'cancelled'`, [payment_method, sessionId]);
            sessionClosed = true;
        }
        await client.query("COMMIT");
