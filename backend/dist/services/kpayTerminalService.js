...
exports.kpayTerminalService = void 0;
...
class KPayTerminalService {
     ...
     */
    buildSignature(method, urlPath, timestamp, nonceStr, body) {
        ...
        const privateKey = crypto_1.default.createPrivateKey(pem);
    ...
    async performSale(paymentDetails) {
        ...
        return new Promise((resolve) => {
            ...
            if (paymentDetails.description)
                bodyFields.description = paymentDetails.description;
     ...
     */
    async queryTransactionStatus(outTradeNo) {
        if (!this.config) {
            return {
                success: false,
                message: 'Service not initialized',
    ...
    async cancelTransaction(details) {
        if (!this.config)
            return { success: false, message: 'Service not initialized', error: 'Configuration missing' };
        return new Promise((resolve) => {
            const timestamp = this.getTimestamp();
            const nonceStr = this.generateNonce();
            const bodyFields = {
            ...
            if (details.callbackUrl)
                bodyFields.callbackUrl = details.callbackUrl;
            const bodyJson = JSON.stringify(bodyFields);
            let signature;
            try {
            ...
            req.on('error', (e) => resolve({ success: false, message: 'Request error', error: e.message }));
    ...
    async refundTransaction(details) {
        ...
        return new Promise((resolve) => {
            ...
            if (details.transactionNo !== undefined)
                bodyFields.transactionNo = details.transactionNo;
            if (details.refNo !== undefined)
                bodyFields.refNo = details.refNo;
            if (details.commitTime !== undefined)
                bodyFields.commitTime = details.commitTime;
            if (details.refundAmount)
                bodyFields.refundAmount = details.refundAmount;
            if (details.callbackUrl)
                bodyFields.callbackUrl = details.callbackUrl;
            const bodyJson = JSON.stringify(bodyFields);
            let signature;
     ...
     */
    encryptManagerPassword(plainPassword) {
        if (!this.platformPublicKey)
            throw new Error('No platform public key — perform key exchange first');
        const raw = this.platformPublicKey.replace(/\s+/g, '');
        const lines = raw.match(/.{1,64}/g).join('\n');
        const pem = `-----BEGIN PUBLIC KEY-----\n${lines}\n-----END PUBLIC KEY-----`;
        const encrypted = crypto_1.default.publicEncrypt({ key: pem, padding: crypto_1.default.constants.RSA_PKCS1_PADDING }, Buffer.from(plainPassword, 'utf8'));
...
exports.kpayTerminalService = new KPayTerminalService();
//# sourceMappingURL=kpayTerminalService.js.map