...
class PaymentAsiaTransactionRepository {
     ...
     */
    async getTransactionByMerchantReference(merchantReference) {
        const query = `
      SELECT
        id, order_id AS "orderId", session_id AS "sessionId",
        restaurant_id AS "restaurantId", merchant_reference AS "merchantReference",
      ...
      LIMIT 1
    `;
        const result = await db_1.default.query(query, [merchantReference]);
        return result.rows.length > 0 ? this.mapRowToTransaction(result.rows[0]) : null;
    }
    /**
     * Get transaction by Payment Asia transaction ID
     */
    async getTransactionByPaymentAsiaId(transactionId) {
        const query = `
      SELECT
        id, order_id AS "orderId", session_id AS "sessionId",
        restaurant_id AS "restaurantId", merchant_reference AS "merchantReference",
      ...
      LIMIT 1
    `;
        const result = await db_1.default.query(query, [transactionId]);
        return result.rows.length > 0 ? this.mapRowToTransaction(result.rows[0]) : null;
    }
    /**
    ...
    async queryTransactions(options) {
        ...
        const countResult = await db_1.default.query(countQuery, values.slice(0, paramCount - 1));
     ...
     */
    async getTransactionStats(restaurantId, dateFrom, dateTo) {
    ...
    `;
        ...
        return {
            ...
            successRate: totalTransactions > 0 ? (approvedCount / totalTransactions) * 100 : 0,
            ...(dateFrom || dateTo ? { datePeriod: { from: dateFrom, to: dateTo } } : {}),
        };
    }
    /**
     * Create a refund transaction
     */
    async createRefund(refundRequest) {
     ...
     */
    async getTransactionsForReconciliation(restaurantId, dateFrom, dateTo) {
        const query = `
      SELECT
        id, order_id AS "orderId", session_id AS "sessionId",
        restaurant_id AS "restaurantId", merchant_reference AS "merchantReference",
    ...
    `;
        const result = await db_1.default.query(query, [restaurantId, dateFrom, dateTo]);
...
exports.default = new PaymentAsiaTransactionRepository();
//# sourceMappingURL=paymentAsiaTransactionRepository.js.map