...
exports.getTerminalSecretCode = getTerminalSecretCode;
exports.getRestaurantTerminals = getRestaurantTerminals;
exports.hasActiveTerminal = hasActiveTerminal;
exports.invalidateTerminalCache = invalidateTerminalCache;
exports.clearTerminalCache = clearTerminalCache;
 ...
 */
async function getActiveTerminal(restaurantId, vendorName) {
    ...
    try {
        const result = await db_1.default.query(`SELECT id, restaurant_id, vendor_name, terminal_id, secret_code, 
              is_active, created_at, updated_at
       FROM payment_terminals
       WHERE restaurant_id = $1 AND vendor_name = $2 AND is_active = true
       LIMIT 1`, [restaurantId, vendorName]);
 ...
 */
async function getTerminalSecretCode(restaurantId, vendorName) {
 ...
 */
async function getRestaurantTerminals(restaurantId) {
    try {
        const result = await db_1.default.query(`SELECT id, restaurant_id, vendor_name, terminal_id, secret_code, 
              is_active, created_at, updated_at
       FROM payment_terminals
       WHERE restaurant_id = $1 AND is_active = true
       ORDER BY vendor_name ASC`, [restaurantId]);
        return result.rows;
    }
    catch (error) {
 ...
 */
async function hasActiveTerminal(restaurantId, vendorName) {
    const terminal = await getActiveTerminal(restaurantId, vendorName);
 ...
 * @param vendorName - Payment vendor name
 */
function invalidateTerminalCache(restaurantId, vendorName) {
    if (vendorName) {
        const cacheKey = `${restaurantId}:${vendorName}`;
        ...
        const keysToDelete = Array.from(terminalCache.keys()).filter(key => key.startsWith(`${restaurantId}:`));
...
//# sourceMappingURL=paymentTerminalService.js.map