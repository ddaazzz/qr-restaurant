...
exports.PrinterQueueService = void 0;
exports.getPrinterQueueService = getPrinterQueueService;
class PrinterQueueService {
     ...
     */
    async addJob(restaurantId, payload, options = {}) {
        const { jobType = 'qr', orderId = null, sessionId = null, priority = 0, printerZoneId = null, maxRetries = 3, } = options;
        const query = `
     ...
     */
    async getJob(jobId) {
        const result = await this.pool.query('SELECT * FROM print_queue WHERE id = $1', [jobId]);
    ...
    async retryJob(jobId) {
        ...
        const result = await this.pool.query(`UPDATE print_queue 
    ...
    async processQueue() {
        ...
        const result = await this.pool.query(`SELECT * FROM print_queue 
    ...
    async processJob(job) {
        ...
        try {
            // Mark as printing
            await this.pool.query('UPDATE print_queue SET status = $1 WHERE id = $2', ['printing', job.id]);
            // Set timeout
            ...
            // Mark as completed
            await this.pool.query('UPDATE print_queue SET status = $1 WHERE id = $2', ['completed', job.id]);
    ...
    async handleJobFailure(job, error) {
        ...
        if (job.retry_count < job.max_retries) {
            ...
            const backoffSeconds = Math.min(this.config.retryIntervalSeconds * Math.pow(2, job.retry_count), this.config.maxRetryDelay);
            const nextRetryAt = new Date(Date.now() + backoffSeconds * 1000);
            await this.pool.query(`UPDATE print_queue 
         SET status = 'queued', 
             retry_count = retry_count + 1,
             next_retry_at = $1,
             error_message = $2
         WHERE id = $3`, [nextRetryAt, errorMessage, job.id]);
            console.log(`[PrinterQueue] Job #${job.id} failed (${job.retry_count + 1}/${job.max_retries} retries). ` +
         ...
         WHERE id = $2`, [errorMessage, job.id]);
            console.error(`[PrinterQueue] Job #${job.id} failed permanently after ${job.max_retries} retries. ` +
                `Error: ${errorMessage}`);
        }
        // Clear timeout
        const timer = this.jobTimers.get(job.id);
     ...
     */
    async clearCompletedJobs(restaurantId, olderThanHours = 24) {
        const result = await this.pool.query(`DELETE FROM print_queue
       WHERE restaurant_id = $1 
       AND status = 'completed'
       AND created_at < CURRENT_TIMESTAMP - INTERVAL '${olderThanHours} hours'`, [restaurantId]);
...
exports.PrinterQueueService = PrinterQueueService;
// Singleton instance (optional)
let queueInstance = null;
function getPrinterQueueService(pool, config) {
    if (!queueInstance) {
        queueInstance = new PrinterQueueService(pool, config);
    }
    return queueInstance;
}
//# sourceMappingURL=printerQueue.js.map