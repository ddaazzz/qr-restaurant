...
router.post('/restaurants/:restaurantId/payment-terminals/:terminalId/cancel', async (req, res) => {
  try {
    ...
    const vendorCheck = await pool.query(
      `SELECT id, vendor_name, app_secret, terminal_ip, terminal_port FROM payment_terminals WHERE id = $1 AND restaurant_id = $2`,
      [terminalId, restaurantId],
    );
    if (vendorCheck.rowCount === 0) return res.status(404).json({ success: false, error: 'Payment terminal not found' });
    ...
    if (tv.vendor_name === 'payment-asia-offline') {
      try {
        const voidResult = await callPAOfflineTerminal(
          { terminal_ip: tv.terminal_ip, terminal_port: tv.terminal_port, app_secret: tv.app_secret },
          'POST', '/order/void',
          { order_id: originOutTradeNo },
        );
        const success = voidResult.ok && voidResult.data?.code === '1000';
        if (success) {
          await pool.query(
            `UPDATE pa_offline_transactions SET status = 'voided', completed_at = NOW(), pa_response = $1 WHERE pa_order_id = $2 AND restaurant_id = $3`,
            [JSON.stringify(voidResult.data?.payload || {}), originOutTradeNo, restaurantId],
          );
          await pool.query(
            `UPDATE orders SET payment_status = 'voided' WHERE chuio_order_reference = $1 AND restaurant_id = $2`,
            [originOutTradeNo, restaurantId],
          );
          try { await pool.query(`UPDATE chuio_payments SET status = 'voided' WHERE order_reference = $1`, [originOutTradeNo]); } catch (_) {}
        }
        return res.json({
          success,
...
router.post('/restaurants/:restaurantId/payment-terminals/:terminalId/refund', async (req, res) => {
  try {
    ...
    // Check vendor to route PA-Offline separately
    const vendorCheckR = await pool.query(
      `SELECT id, vendor_name, app_secret, terminal_ip, terminal_port FROM payment_terminals WHERE id = $1 AND restaurant_id = $2`,
      [terminalId, restaurantId],
    );
    if (vendorCheckR.rowCount === 0) return res.status(404).json({ success: false, error: 'Payment terminal not found' });
    ...
    if (tvr.vendor_name === 'payment-asia-offline') {
      ...
      if (!paOrderId) return res.status(400).json({ error: 'originOutTradeNo (pa_order_id) is required for PA-Offline refund' });
      try {
        const refBody: any = { order_id: paOrderId };
        if (refundAmount) refBody.amount = String(refundAmount);
        const refResult = await callPAOfflineTerminal(
          { terminal_ip: tvr.terminal_ip, terminal_port: tvr.terminal_port, app_secret: tvr.app_secret },
          'POST', '/order/refund',
          refBody,
        );
        ...
        if (success) {
          ...
          const dbRes = await pool.query(
            `UPDATE pa_offline_transactions SET status = $1, completed_at = NOW(), pa_response = $2 WHERE pa_order_id = $3 AND restaurant_id = $4 RETURNING amount_cents`,
            [refStatus, JSON.stringify(refResult.data?.payload || {}), paOrderId, restaurantId],
          );
          await pool.query(
            `UPDATE orders SET payment_status = $1 WHERE chuio_order_reference = $2 AND restaurant_id = $3`,
            [refStatus, paOrderId, restaurantId],
          );
          try {
            const refAmtCents = refundAmount ? Math.round(parseFloat(String(refundAmount)) * 100) : (dbRes.rows[0]?.amount_cents || 0);
            await pool.query(
              `UPDATE chuio_payments SET status = $1, refund_amount_cents = $2, refunded_at = NOW() WHERE order_reference = $3`,
              [refStatus, refAmtCents, paOrderId],
            );
          } catch (_) {}
        }
        return res.json({
...
router.get('/restaurants/:restaurantId/pa-offline-transactions/:pa_order_id', async (req, res) => {
  try {
    ...
    const dbRes = await pool.query(
      `SELECT id, pa_order_id, amount_cents, currency, payment_method,
