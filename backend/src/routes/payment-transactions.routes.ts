...
const testPaymentWebhooks = new Map<string, any>();
 ...
 */
router.post(
  '/restaurants/:restaurantId/sessions/:sessionId/orders/:orderId/initiate-payment',
  async (req, res) => {
    try {
      ...
      const restaurantRes = await pool.query(
        `SELECT active_payment_vendor, active_payment_terminal_id, service_charge_percent FROM restaurants WHERE id = $1`,
        [restaurantId]
      );
      ...
      const terminalRes = await pool.query(
        `SELECT id, merchant_token, secret_code, payment_gateway_env FROM payment_terminals
         WHERE id = $1 AND restaurant_id = $2 AND vendor_name = 'payment-asia'`,
        [restaurant.active_payment_terminal_id, restaurantId]
      );
      ...
      const orderRes = await pool.query(
        `SELECT o.id, o.session_id, o.restaurant_id,
         ...
         GROUP BY o.id, o.session_id, o.restaurant_id`,
        [orderId, sessionId, restaurantId]
      );
      ...
      await pool.query(
        `UPDATE orders SET chuio_order_reference = $1, payment_method = 'payment-asia' WHERE id = $2`,
        [chuioRef, orderId]
      );
      ...
      if (existingPaymentRes.rowCount != null && existingPaymentRes.rowCount > 0) {
        ...
        await pool.query(
          `UPDATE order_payments SET amount_cents = $1, currency = 'HKD',
           chuio_order_reference = $2, updated_at = NOW() WHERE id = $3`,
          [totalCents, chuioRef, existingPaymentRes.rows[0].id]
        );
        paymentRecord = existingPaymentRes.rows[0];
      } else {
        const createPaymentRes = await pool.query(
      ...
      const formData = paymentAsiaService.buildPaymentForm({
        orderId: chuioRef,
        amount: totalCents,
        currency: 'HKD',
      ...
      const paymentUrl = paymentAsiaService.getPaymentFormUrl(network);
      ...
      await pool.query(
        `UPDATE order_payments 
         SET signature = $1, payment_url = $2, response_data = $3,
             updated_at = NOW()
         WHERE id = $4`,
        [formData.sign, paymentUrl, JSON.stringify(formData), paymentRecord.id]
      );

      // Log to payment_asia_transactions for audit trail
      try {
        const paymentAsiaTransaction = await paymentAsiaTransactionRepository.createTransaction({
...
router.post('/restaurants/:restaurantId/webhook/payment-asia', async (req, res) => {
  try {
    ...
    const paymentRes = await pool.query(
      `SELECT op.id, op.order_id, op.session_id FROM order_payments op
    ...
    const terminalRes = await pool.query(
      `SELECT secret_code FROM payment_terminals 
       WHERE restaurant_id = $1 AND vendor_name = 'payment-asia' AND is_active = true`,
      [restaurantId]
    );
    ...
    // Verify signature
    const isSignatureValid = paymentAsiaService.verifyWebhookSignature(
      payload,
      receivedSignature,
      terminal.secret_code
    );
    ...
    await pool.query(
      `UPDATE order_payments 
       SET payment_status = $1, webhook_received_at = NOW(), 
           ...
           updated_at = NOW()
       WHERE id = $6`,
      [paymentStatus, completedAt, failedAt, errorMessage, JSON.stringify(payload), payment.id]
    );
    ...
    try {
      ...
      const transaction = await paymentAsiaTransactionRepository
        .getTransactionByMerchantReference(payload.merchant_reference);

      if (transaction) {
        const updateData: Parameters<typeof paymentAsiaTransactionRepository.updateTransactionStatus>[1] = {
        ...
        await paymentAsiaTransactionRepository.updateTransactionStatus(transaction.id, updateData);
        ...
        const newTransaction = await paymentAsiaTransactionRepository.createTransaction({
          restaurantId: parseInt(restaurantId),
        ...
        await paymentAsiaTransactionRepository.updateTransactionStatus(newTransaction.id, updateData);
    ...
    if (paymentStatus === 'completed') {
      ...
      try {
        ...
        const _paNetLabels: Record<string,string> = { Alipay:'Alipay', Wechat:'WeChat Pay', CreditCard:'Credit Card', Octopus:'Octopus', Fps:'FPS', CUP:'UnionPay' };
      ...
      try {
        webSocketServer.broadcastOrderStatusChange({
 ...
 */
async function handlePaymentReturn(req: any, res: any) {
  try {
    const { restaurantId } = req.params;
    ...
    try {
      ...
      if (transaction) {
        ...
        if (requestRef) updateData.transactionId = requestRef;
        if (transactionStatus === 'approved') updateData.approvedAt = new Date();
        if (transactionStatus === 'failed') updateData.failedAt = new Date();
        await paymentAsiaTransactionRepository.updateTransactionStatus(transaction.id, updateData);
      }
    } catch (auditError) {
 ...
 */
function normalizePaymentAsiaStatus(status: string | undefined): string {
  const statusStr = String(status).toLowerCase().trim();
  
  // Map numeric codes
  if (statusStr === '1' || statusStr === 'success' || statusStr === 'approved') return 'success';
  if (statusStr === '0' || statusStr === 'failed' || statusStr === 'cancelled' || statusStr === 'rejected') return 'failed';
...
function closeTabHtml(status: string): string {
...
router.get('/payment-callback', (req: Request, res: Response) => {
  try {
    const rawStatus = req.query.status as string || 'unknown';
    const status = normalizePaymentAsiaStatus(rawStatus);
    const message = req.query.message as string || 'Transaction processed';
    const reference = req.query.reference as string || 'N/A';
    ...
    if (merchantReference) {
      ...
      const signature = (req.query.sign as string) || (req.query.Sign as string) || '';
      
      testPaymentWebhooks.set(merchantReference as string, {
        merchant_reference: merchantReference,
        request_reference: reference || (req.query.request_reference as string),
        currency: (req.query.currency as string) || 'HKD',
        amount: (req.query.amount as string) || 'unknown',
        status: status,
        sign: signature,
  ...
  try {
    ...
    if (merchantReference) {
      ...
      testPaymentWebhooks.set(merchantReference as string, {
        ...
        currency: (req.body?.currency || req.query?.currency) as string || 'HKD',
        amount: (req.body?.amount || req.query?.amount) as string || 'unknown',
        status: status,
        sign: signature,
        received_at: new Date().toISOString(),
      });
    ...
    if (!returnTo) returnTo = req.query.return_to as string;
 ...
 */
router.post('/webhook/payment-asia-test', (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/webhook/payment-test-status/:merchantReference', (req: Request, res: Response) => {
  try {
    const merchantReference = req.params.merchantReference as string;
 ...
 * }
 */
router.post('/webhook/payment-test-trigger/:merchantReference', (req: Request, res: Response) => {
  try {
    const merchantReference = req.params.merchantReference as string;
...
async function initPaymentAsiaForRestaurant(restaurantId: string, terminalId?: string) {
  let resolvedTerminalId = terminalId;

  if (!resolvedTerminalId) {
    // Fall back to the restaurant's active terminal
    const restaurantRes = await pool.query(
      `SELECT active_payment_terminal_id FROM restaurants WHERE id = $1`,
      [restaurantId]
    );
    if (restaurantRes.rowCount === 0) {
  ...
  const terminalRes = await pool.query(
    `SELECT merchant_token, secret_code, payment_gateway_env FROM payment_terminals
     WHERE id = $1 AND restaurant_id = $2 AND vendor_name = 'payment-asia'`,
    [resolvedTerminalId, restaurantId]
  );
  if (terminalRes.rowCount === 0) {
    throw Object.assign(new Error('Payment Asia terminal not configured for this restaurant'), { status: 400 });
  }
...
router.post('/restaurants/:restaurantId/payment-asia/query', async (req, res) => {
  try {
    ...
    if (!merchant_reference) return res.status(400).json({ error: 'merchant_reference is required' });

    await initPaymentAsiaForRestaurant(restaurantId, terminal_id);
    const records = await paymentAsiaService.queryPayment(merchant_reference);
    ...
    const completedSale = records.find((r: any) => isSaleRecord(r) && String(r.status) === '1');

    let synced = false;
    if (completedSale) {
      // Look up order_payments via chuio_order_reference (= merchant_reference sent to PA)
      const paymentRes = await pool.query(
        `SELECT id, order_id FROM order_payments
         WHERE chuio_order_reference = $1 AND restaurant_id = $2
         LIMIT 1`,
        [merchant_reference, restaurantId]
      );
...
router.post('/restaurants/:restaurantId/payment-asia/refund', async (req, res) => {
  try {
    ...
    await initPaymentAsiaForRestaurant(restaurantId, terminal_id);
    const result = await paymentAsiaService.requestRefund(merchant_reference, amountNum);
...
router.post('/restaurants/:restaurantId/payment-asia/refund-query', async (req, res) => {
  try {
    ...
    if (!refund_reference) return res.status(400).json({ error: 'refund_reference is required' });

    await initPaymentAsiaForRestaurant(restaurantId, terminal_id);
    const result = await paymentAsiaService.queryRefund(refund_reference);
    // PA success codes: '200' (production JSON) or '20000' (sandbox/form-encoded)
    const paCode = String(result?.response?.code ?? '');
    const paSuccess = !paCode || paCode === '200' || paCode === '20000';
    if (!paSuccess) {
