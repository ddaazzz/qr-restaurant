...
// ─── POST /api/xish/members/:memberId/redeem-points ─────────────────────────
// Deduct points and issue a gift card OR a coupon as reward.
// Body: { restaurant_id, reward_type: 'gift_card'|'coupon', points_cost,
//         gift_card_value_cents?, coupon_code?, coupon_discount_type?,
//         coupon_discount_value?, coupon_description? }
router.post("/xish/members/:memberId/redeem-points", async (req, res) => {
  const client = await pool.connect();
  try {
    const { memberId } = req.params;
    const {
    ...
    if (!restaurant_id || !reward_type || !points_cost) {
      return res.status(400).json({ error: "restaurant_id, reward_type, and points_cost are required" });
    }
    if (points_cost <= 0) {
      return res.status(400).json({ error: "points_cost must be positive" });
    }

    // Verify member exists and has enough points
    const memberRes = await client.query(
      `SELECT m.id, m.points_balance, c.id AS crm_id
       FROM xish_members m
       JOIN crm_customers c ON c.id = m.crm_customer_id
       WHERE m.id = $1 AND c.restaurant_id = $2`,
      [memberId, restaurant_id]
    );
    if (!memberRes.rows[0]) return res.status(404).json({ error: "Member not found" });
    if (memberRes.rows[0].points_balance < points_cost) {
      return res.status(400).json({ error: `Insufficient points. Balance: ${memberRes.rows[0].points_balance}` });
    }
    ...
    await client.query(
      `INSERT INTO xish_point_transactions (member_id, restaurant_id, points_delta, reason)
       VALUES ($1, $2, $3, $4)`,
      [memberId, restaurant_id, -points_cost, `redeem_${reward_type}`]
    );

    const updated = await client.query(
      `UPDATE xish_members SET points_balance = points_balance - $2, updated_at = NOW()
       WHERE id = $1 RETURNING points_balance, tier`,
      [memberId, points_cost]
    );

    let reward: Record<string, any> = {};

    if (reward_type === "gift_card") {
      if (!gift_card_value_cents || gift_card_value_cents <= 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({ error: "gift_card_value_cents required for gift card reward" });
      }
      ...
      if (!code) { await client.query("ROLLBACK"); return res.status(500).json({ error: "Could not generate code" }); }

      const gcRes = await client.query(
        `INSERT INTO gift_cards (restaurant_id, code, original_value_cents, balance_cents, note)
         VALUES ($1, $2, $3, $3, $4) RETURNING *`,
        [restaurant_id, code, gift_card_value_cents, `Redeemed with ${points_cost} loyalty pts`]
      );
      reward = { type: "gift_card", ...gcRes.rows[0] };
    } else if (reward_type === "coupon") {
      if (!coupon_code || !coupon_discount_type || !coupon_discount_value) {
        await client.query("ROLLBACK");
        return res.status(400).json({ error: "coupon_code, coupon_discount_type, and coupon_discount_value required" });
      }
      const cpRes = await client.query(
        `INSERT INTO coupons (restaurant_id, code, discount_type, discount_value, description, coupon_type, max_uses)
         VALUES ($1, UPPER($2), $3, $4, $5, 'closed', 1)
         RETURNING *`,
        [restaurant_id, coupon_code, coupon_discount_type, coupon_discount_value,
         coupon_description || `Redeemed with ${points_cost} loyalty pts`]
      ).catch(async (err: any) => {
        if (err.message?.includes("coupon_type")) {
          return client.query(
            `INSERT INTO coupons (restaurant_id, code, discount_type, discount_value, description, max_uses)
             VALUES ($1, UPPER($2), $3, $4, $5, 1) RETURNING *`,
            [restaurant_id, coupon_code, coupon_discount_type, coupon_discount_value,
             coupon_description || `Redeemed with ${points_cost} loyalty pts`]
          );
        }
      ...
      reward = { type: "coupon", ...cpRes.rows[0] };
    } else {
      await client.query("ROLLBACK");
      return res.status(400).json({ error: "reward_type must be 'gift_card' or 'coupon'" });
    }

    await client.query("COMMIT");

    res.json({
      points_balance: updated.rows[0].points_balance,
      tier: updated.rows[0].tier,
      reward,
    });
  } catch (err: any) {
    await client.query("ROLLBACK");
    console.error("[XISH redeem-points]", err);
    if (err.code === "23505") {
      return res.status(400).json({ error: "Coupon code already exists, choose a different code" });
    }
    res.status(500).json({ error: "Internal server error" });
  } finally {
    client.release();
